...
You would ahve to install the micronucleus tool in your pc for flashing code to the Attiny85. Commands for building and flashing the code is present in the [Makefile](Makefile). Since I have done this part in Linux, I'm not exactly sure of the processes to install the micronucleus and other dependencies in Mac or Windows, so the below part is specifically for linux. 
